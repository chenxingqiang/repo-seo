#!/usr/bin/env python3
"""
Google Gemini provider implementation for the GitHub Repository SEO Optimizer.

This module provides an implementation of the BaseProvider interface
that uses the Google Gemini API to generate content.
"""
...
try:
    ...
    logger.warning("Google Generative AI library not available. Install it with: pip install google-generativeai")


class GeminiProvider(BaseProvider):
    """Google Gemini provider implementation that uses the Google Gemini API.
    
    This provider requires a Google API key to be set in the
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "gemini-pro", 
        ...
        Args:
            api_key: The Google API key, or None to use the GOOGLE_API_KEY or GEMINI_API_KEY environment variable.
        ...
        if not GEMINI_AVAILABLE:
            raise ImportError(
                "Google Generative AI library not available. Install it with: pip install google-generativeai"
            )
        
        self.api_key = api_key or os.environ.get("GOOGLE_API_KEY") or os.environ.get("GEMINI_API_KEY")
        if not self.api_key:
            raise ValueError(
                "Google API key not provided. Set the GOOGLE_API_KEY or GEMINI_API_KEY environment variable."
            )
    ...
    def generate_description(self, repo_name: str, languages: List[str], 
        ...
        """Generate a description for a repository using the Google Gemini API.
    ...
    def generate_topics(self, repo_name: str, languages: List[str], 
        ...
        """Generate topics for a repository using the Google Gemini API.
        ...
        try:
            ...
            logger.error(f"Error parsing topics from Google Gemini response: {e}")
    ...
    def analyze_readme(self, readme: str) -> Dict[str, Any]:
        """Analyze the content of a README file using the Google Gemini API.
        ...
        try:
            ...
            if start_idx >= 0 and end_idx > start_idx:
                ...
                logger.error("No JSON object found in Google Gemini response")
                logger.debug(f"Response content: {content}")
            ...
            logger.error(f"Error parsing analysis from Google Gemini response: {e}")
            logger.debug(f"Response content: {content}")
    ...
    def generate_readme(self, repo_name: str, languages: List[str], 
        ...
        """Generate a README file for a repository using the Google Gemini API.
        ...
        generation_config = {
            "temperature": self.temperature,
            "max_output_tokens": 1500,  # Increase token limit for README generation
        }
