"""
Zhipu Language Model Provider for GitHub SEO Optimizer

This provider allows using Zhipu's models.
"""
...
class ZhipuProvider(LLMProvider):
    """Zhipu language model provider."""

    def __init__(self, api_key: Optional[str] = None, model: str = "chatglm_turbo", **kwargs):
        """
        Initialize the Zhipu provider.

        Args:
            api_key: Zhipu API key. If not provided, will try to get from environment.
            model: Model to use for generation. Default is "chatglm_turbo".
            **kwargs: Additional arguments to pass to the Zhipu API.
        """
        self.api_key = api_key or os.environ.get("ZHIPU_API_KEY")
        if not self.api_key:
            raise ValueError("Zhipu API key not provided and not found in environment")

        self.model = model
        self.api_base = kwargs.pop("api_base", "https://open.bigmodel.cn/api/paas/v3/model-api")
        self.kwargs = kwargs

    def _generate_token(self) -> str:
        """Generate authentication token for Zhipu API."""
        try:
            # Split API key into ID and secret
            id_and_secret = self.api_key.split(".")
            if len(id_and_secret) != 2:
                return ""

            api_id, api_secret = id_and_secret
            ...
            # Create JWT token
            jwt_token = f"{api_id}.{base64.b64encode(payload_str.encode('utf-8')).decode('utf-8')}.{signature_b64}"

            return jwt_token
        except Exception as e:
            print(f"Error generating Zhipu token: {str(e)}")
            return ""

    def _generate_completion(self, prompt: str, max_tokens: int = 500) -> str:
        """
        Generate a completion using the Zhipu API.
        ...
        try:
            ...
            if not token:
                return "Error: Failed to generate authentication token for Zhipu API."
            ...
            response = requests.post(
                f"{self.api_base}/{self.model}/sse-invoke",
                headers=headers,
                json=data,
                timeout=30
            )
            ...
            if response.status_code == 200:
                ...
                error_msg = f"Error from Zhipu API: {response.status_code}"
                try:
            ...
            return "Error: Could not connect to Zhipu API."
        except requests.exceptions.Timeout:
            return "Error: Request to Zhipu API timed out."
        except Exception as e:
            return f"Error generating completion with Zhipu: {str(e)}"
