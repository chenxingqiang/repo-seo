"""
Qianwen Language Model Provider for GitHub SEO Optimizer

This provider allows using Alibaba Cloud's Qianwen models.
"""
...
class QianwenProvider(LLMProvider):
    """Qianwen language model provider."""
    
    def __init__(self, api_key: Optional[str] = None, model: str = "qwen-turbo", **kwargs):
        """
        Initialize the Qianwen provider.
        
        Args:
            api_key: Qianwen API key. If not provided, will try to get from environment.
            model: Model to use for generation. Default is "qwen-turbo".
            **kwargs: Additional arguments to pass to the Qianwen API.
        """
        self.api_key = api_key or os.environ.get("QIANWEN_API_KEY")
        if not self.api_key:
            raise ValueError("Qianwen API key not provided and not found in environment")
        
        self.model = model
        self.api_base = kwargs.pop("api_base", "https://dashscope.aliyuncs.com/api/v1/services/aigc/text-generation/generation")
        self.kwargs = kwargs
    
    def _generate_completion(self, prompt: str, max_tokens: int = 500) -> str:
        """
        Generate a completion using the Qianwen API.
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                error_msg = f"Error from Qianwen API: {response.status_code}"
                try:
            ...
            return "Error: Could not connect to Qianwen API."
        except requests.exceptions.Timeout:
            return "Error: Request to Qianwen API timed out."
        except Exception as e:
            return f"Error generating completion with Qianwen: {str(e)}"
